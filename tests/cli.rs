...
fn version_json_reports_api_contract() {
    let output = Command::new(portal_proxy())
        .args(["version", "--json"])
        .output()
        .expect("run portal-proxy version");
...
fn list_v1_returns_empty_sessions_for_empty_state() {
    let state_dir = std::env::temp_dir().join(format!("portal-proxy-cli-{}", uuid::Uuid::new_v4()));
    let output = Command::new(portal_proxy())
        .args([
            "--state-dir",
